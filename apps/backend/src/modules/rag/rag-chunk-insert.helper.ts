 ...
 const placeholders: string[] = [];
    ...
    if (hasVec) {
      slice.forEach((r, idx) => {
        ...
        params.push(
          r.documentId, r.versionId, r.chunkIndex, clean(r.content), clean(r.rawText),
          r.tokenCount, clean(r.breadcrumb), clean(r.lawName), clean(r.lawNumber),
          clean(r.chapter), clean(r.section), clean(r.article), clean(r.clause), clean(r.point),
          r.charStart, r.charEnd, r.embeddingVec ? `[${r.embeddingVec.join(',')}]` : null,
        );
      });
      await ds.query(
      ...
      slice.forEach((r, idx) => {
        ...
        params.push(
          ...
          r.tokenCount, clean(r.breadcrumb), clean(r.lawName), clean(r.lawNumber),
          clean(r.chapter), clean(r.section), clean(r.article), clean(r.clause), clean(r.point),
          r.charStart, r.charEnd, r.embeddingVec ? JSON.stringify(r.embeddingVec) : null,
        );
      });
      await ds.query(
        `INSERT INTO rag_chunks (
