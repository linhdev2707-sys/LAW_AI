...
export async function bulkInsertChunks(ds: DataSource, rows: IInsertRow[]): Promise<void> {
 ...
 r.content ?? '', r.rawText ?? r.content ?? '',
 r.tokenCount ?? 0,
 r.breadcrumb ?? '',
 r.lawName ?? null, r.lawNumber ?? null,
 r.chapter ?? null, r.section ?? null,
 // `article` is NOT NULL in the entity — fall back to '?'
 // so an empty chunk (e.g. preamble) never violates the
 // constraint and crashes the whole upload.
 r.article || '?',
 r.clause ?? null, r.point ?? null,
 r.charStart ?? null, r.charEnd ?? null,
