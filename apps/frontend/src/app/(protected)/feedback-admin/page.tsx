...
const QUESTION_MAP: Record<string, { label: string; type: 'text' | 'scale5' | 'scale10' | 'list' }> = {
  q1: { label: '1. Bạn thuộc nhóm người dùng nào?', type: 'text' },
  q2: { label: '2. Trước khi sử dụng iLaw, mức độ hiểu biết của bạn về pháp luật là:', type: 'text' },
  q3: { label: '3. Khi sử dụng iLaw, bạn đã thử hỏi hoặc quan tâm đến lĩnh vực pháp luật nào?', type: 'list' },
  q4: { label: '4. Trước khi dùng iLaw, bạn thường tìm thông tin pháp luật bằng cách nào?', type: 'list' },
  ...
  q7: { label: '7. Giao diện website iLaw có dễ nhìn, rõ ràng và chuyên nghiệp không?', type: 'scale5' },
  ...
  q13: { label: '13. Câu trả lời của iLaw có trình bày bằng ngôn ngữ dễ hiểu không?', type: 'scale5' },
  q14: { label: '14. Khi trải nghiệm nội dung tư vấn, bạn nhận thấy những yếu tố nào?', type: 'list' },
  ...
  q19: { label: '19. Đánh giá mức độ dễ tìm/dễ hiểu của trang điều khoản & chính sách:', type: 'text' },
...
export default function FeedbackAdminPage() {
  ...
  if (sessionStatus === 'loading' || !isAdmin) {
    return (
      <main className="relative min-h-[60vh] bg-brand-background text-brand-on-surface-variant flex items-center justify-center">
  ...
  const renderAnswerDetail = (key: string, value: any, type: string) => {
    if (value === undefined || value === null) {
      return <span className="text-brand-on-surface-variant/40 italic">Không trả lời</span>;
    }
    ...
    if (type === 'list') {
      ...
      if (items.length === 0) return <span className="text-brand-on-surface-variant/40 italic">Không chọn mục nào</span>;
      return (
        <ul className="mt-1 space-y-1">
          {items.map((item: string, idx: number) => (
            <li key={idx} className="flex items-start gap-1.5 text-sm text-brand-on-surface-variant">
              <CheckCircle className="h-4 w-4 shrink-0 text-brand-tertiary mt-0.5" />
              <span>{item}</span>
            </li>
          ))}
        </ul>
    ...
    return (
      <p className="mt-1 text-sm text-brand-on-surface-variant whitespace-pre-line leading-relaxed">
        {value}
      </p>
    );
  };
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-brand-background text-brand-on-surface">
      <div aria-hidden className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_80%_50%_at_50%_0%,rgba(0,229,255,0.18),transparent_60%)]" />
      ...
      <div className="container relative max-w-6xl py-12">
        ...
        {/* Toolbar & Filters */}
        <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4 rounded-xl border border-brand-outline-variant/15 bg-brand-surface-container/60 p-4 backdrop-blur-md">
          {/* Search */}
          <div className="group relative w-full md:max-w-xs">
            ...
            <Input
              ...
              className="pl-9 border-brand-outline-variant/30 bg-brand-surface-container-lowest/60 focus-visible:border-brand-primary focus-visible:ring-brand-primary/30 text-brand-on-surface placeholder:text-brand-on-surface-variant/40"
            />
          </div>
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-brand-on-surface-variant font-medium">Lọc:</span>
            </div>
            ...
            <div className="relative w-52">
              <select
                ...
                className="h-10 w-full appearance-none rounded-md border border-brand-outline-variant/30 bg-brand-surface-container-lowest/60 pl-4 pr-10 py-2 text-sm text-brand-on-surface focus-visible:outline-none focus-visible:border-brand-primary focus-visible:ring-2 focus-visible:ring-brand-primary/20 cursor-pointer"
              >
            ...
            <Button
              ...
              disabled={loading}
              className="h-10 text-brand-on-surface-variant hover:bg-white/5 hover:text-brand-primary transition-colors border border-brand-outline-variant/10"
            >
              Làm mới
            </Button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-brand-outline-variant/10">
                {loading ? (
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-brand-on-surface-variant">
                  ...
                  <tr>
                    <td colSpan={6} className="px-6 py-12 text-center text-sm text-brand-on-surface-variant">
                  ...
                  filteredFeedbacks.map((fb) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-6 py-4 text-sm text-brand-on-surface-variant font-medium whitespace-nowrap">
                          {formatDate(fb.createdAt)}
                        </td>
                        <td className="px-6 py-4">
                          {fb.user ? (
                            <div className="flex flex-col">
                              <span className="text-sm font-semibold text-brand-on-surface truncate max-w-[200px]">
                                {fb.user.fullName}
                              </span>
                              <span className="text-xs text-brand-on-surface-variant/80 truncate max-w-[200px]">
                                {fb.user.email}
                              </span>
                            </div>
                          ) : (
                            <span className="text-sm font-medium text-brand-on-surface-variant/60 italic">
                              Khách (Ẩn danh)
                            </span>
                          )}
                        </td>
                        <td className="px-6 py-4 text-center">
                          <span className={cn(
                            "inline-flex items-center justify-center h-7 w-12 rounded-full text-xs font-bold",
                            interfaceScore >= 4 
                        ...
                        <td className="px-6 py-4 text-right">
                          <button
                            type="button"
                            className="inline-flex h-8 w-8 items-center justify-center rounded-lg text-brand-on-surface-variant hover:bg-white/5 hover:text-brand-primary transition-all group-hover:translate-x-0.5"
                          >
      ...
      {selectedFeedback && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div 
            ...
            className="absolute inset-0 bg-black/75 backdrop-blur-xs animate-in fade-in-0 duration-200"
          />

          {/* Drawer content */}
          <div className="relative z-10 flex h-full w-full max-w-xl flex-col bg-brand-surface-container shadow-2xl border-l border-brand-outline-variant/20 animate-in slide-in-from-right duration-300">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-brand-outline-variant/15 px-6 py-4 bg-white/[0.01]">
              <div className="min-w-0">
                <h3 className="text-lg font-bold text-brand-on-surface truncate">
              ...
              <button
                type="button"
                onClick={() => setSelectedFeedback(null)}
                className="rounded-full p-1.5 text-brand-on-surface-variant hover:bg-white/5 hover:text-brand-on-surface transition-colors"
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="flex items-center gap-4 rounded-xl border border-brand-outline-variant/10 bg-white/[0.01] p-4">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-brand-primary/10 text-brand-primary border border-brand-primary/20">
                  <UserIcon className="h-5 w-5" />
                </span>
                <div className="min-w-0">
                  <h4 className="text-sm font-semibold uppercase tracking-wider text-brand-on-surface-variant">Người đóng góp</h4>
                  {selectedFeedback.user ? (
                    <div className="mt-0.5">
                      <p className="text-sm font-bold text-brand-on-surface">{selectedFeedback.user.fullName}</p>
                      <p className="text-xs text-brand-on-surface-variant/80">{selectedFeedback.user.email}</p>
                    </div>
                  ) : (
                    <p className="text-sm font-medium text-brand-on-surface-variant/70 italic mt-0.5">Khách (Ẩn danh)</p>
                  )}
                </div>
              </div>
              ...
              {SECTIONS.map((sec, idx) => (
                <div key={idx} className="space-y-4">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-brand-tertiary border-b border-brand-tertiary/10 pb-1.5">
                    {sec.title}
                  </h4>
                    ...
                    {sec.fields.map((field) => {
                      ...
                      return (
                        <div key={field} className="space-y-1 text-left">
                          <p className="text-sm font-semibold text-brand-on-surface leading-normal">
                            {qMeta?.label || `${field.toUpperCase()}:`}
                          </p>
                          <div className="pl-3 py-1 border-l-2 border-brand-outline-variant/15">
            ...
            {/* Footer */}
            <div className="border-t border-brand-outline-variant/15 px-6 py-4 bg-white/[0.01] flex justify-end">
              <Button
                type="button"
                onClick={() => setSelectedFeedback(null)}
                className="bg-white/10 hover:bg-white/15 text-brand-on-surface font-semibold"
              >
                Đóng
              </Button>
            </div>
