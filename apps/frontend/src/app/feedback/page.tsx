...
export default function FeedbackPage() {
  ...
  const validateForm = () => {
    ...
    if (q3.length === 0 && !q3IsOther)
      errors.push('Câu 3: Vui lòng chọn lĩnh vực pháp luật bạn quan tâm.');
    if (q3IsOther && !q3Other.trim()) errors.push('Câu 3: Vui lòng điền thông tin cho mục khác.');

    if (q4.length === 0 && !q4IsOther)
      errors.push('Câu 4: Vui lòng chọn cách tìm kiếm thông tin pháp luật trước đây.');
    if (q4IsOther && !q4Other.trim()) errors.push('Câu 4: Vui lòng điền thông tin cho mục khác.');

    if (q5.length === 0 && !q5IsOther)
      errors.push('Câu 5: Vui lòng chọn những phần đã trải nghiệm trên iLaw.');
    ...
    if (q7 === null)
      errors.push('Câu 7: Vui lòng đánh giá mức độ chuyên nghiệp/dễ nhìn của giao diện.');
    if (q8 === null)
      errors.push('Câu 8: Vui lòng đánh giá mức độ dễ hiểu về dịch vụ iLaw cung cấp.');
    ...
    if (q12.length === 0 && !q12IsOther)
      errors.push('Câu 12: Vui lòng chọn tình huống iLaw hữu ích nhất.');
    if (q12IsOther && !q12Other.trim())
      errors.push('Câu 12: Vui lòng điền thông tin cho mục khác.');

    if (q13 === null) errors.push('Câu 13: Vui lòng đánh giá ngôn ngữ trình bày của iLaw.');

    // New validations Q14 - Q25
    if (q14.length === 0 && !q14IsOther)
      errors.push('Câu 14: Vui lòng chọn yếu tố nhận thấy khi trải nghiệm nội dung tư vấn.');
    if (q14IsOther && !q14Other.trim())
      errors.push('Câu 14: Vui lòng điền thông tin cho mục khác.');

    if (!q15 && !q15IsOther) errors.push('Câu 15: Vui lòng chọn phần tạo ấn tượng tốt nhất.');
    if (q15IsOther && !q15Other.trim())
      errors.push('Câu 15: Vui lòng điền thông tin cho mục khác.');

    // Part 4
    if (q16 === null) errors.push('Câu 16: Vui lòng đánh giá độ tin tưởng câu trả lời.');

    if (q17.length === 0 && !q17IsOther)
      errors.push('Câu 17: Vui lòng chọn yếu tố khiến bạn cảm thấy tin tưởng hơn.');
    if (q17IsOther && !q17Other.trim())
      errors.push('Câu 17: Vui lòng điền thông tin cho mục khác.');

    if (q18 === null) errors.push('Câu 18: Vui lòng đánh giá độ lo ngại về sai sót/chưa cập nhật.');

    if (!q19)
      errors.push('Câu 19: Vui lòng đánh giá mức độ dễ tìm/dễ hiểu phần điều khoản và chính sách.');
    ...
    if (!q25.trim())
      errors.push('Câu 25: Vui lòng đề xuất một thay đổi quan trọng nhất để iLaw tốt hơn.');
  ...
  if (submitted) {
    return (
      <div className="flex min-h-screen flex-col justify-between bg-brand-background text-brand-on-surface">
        <LandingNavbar />
        <div className="flex flex-1 items-center justify-center px-4 pb-16 pt-32">
          <div className="relative w-full max-w-xl text-center">
            <div
              aria-hidden
              className="absolute -inset-10 z-0 bg-gradient-to-r from-brand-primary/20 via-brand-tertiary/20 to-transparent opacity-50 blur-3xl"
            />

            <div className="relative z-10 flex flex-col items-center rounded-2xl border border-brand-outline-variant/30 bg-brand-surface-container/60 p-8 shadow-2xl backdrop-blur-md md:p-12">
              ...
              <p className="mt-4 text-base leading-relaxed text-brand-on-surface-variant">
                iLaw chân thành cảm ơn những ý kiến đóng góp quý báu của bạn. Sự hỗ trợ của bạn sẽ
                giúp hệ thống ngày càng hoàn thiện, chính xác và hữu ích hơn cho cộng đồng.
              </p>
              ...
              <div className="mt-8 flex w-full flex-col justify-center gap-3 sm:flex-row">
                <button
                  ...
                  className="flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-brand-primary to-brand-tertiary px-6 py-3 text-sm font-semibold text-brand-surface-container-lowest shadow-lg shadow-brand-primary/20 transition-all hover:scale-[1.02] active:scale-[0.98]"
                >
                ...
                <button
                  type="button"
                  onClick={() => router.push('/')}
                  className="flex items-center justify-center gap-2 rounded-lg border border-brand-outline-variant/30 bg-white/[0.03] px-6 py-3 text-sm font-semibold text-brand-on-surface transition-all hover:bg-white/[0.07]"
                >
                  Về Trang chủ
                </button>
              </div>
  ...
  return (
    <div className="flex min-h-screen flex-col justify-between bg-brand-background text-brand-on-surface">
      <LandingNavbar />
      <div className="flex-1 px-4 pb-16 pt-32 md:px-8">
        {/* Decorative glows */}
        <div className="pointer-events-none fixed -top-40 right-0 -z-10 h-96 w-96 rounded-full bg-brand-primary/10 blur-[100px]" />
        <div className="pointer-events-none fixed -bottom-40 left-0 -z-10 h-96 w-96 rounded-full bg-brand-tertiary/10 blur-[100px]" />
        ...
        <div className="mx-auto max-w-3xl">
          ...
          <button
            type="button"
            onClick={() => router.back()}
            className="group mb-8 flex items-center gap-2 text-sm font-semibold text-brand-on-surface-variant transition-colors duration-200 hover:text-brand-primary"
          >
          ...
          {/* Title Card */}
          <div className="relative mb-8 overflow-hidden rounded-2xl border border-brand-primary/20 bg-brand-surface-container/60 p-6 shadow-2xl backdrop-blur-md md:p-8">
            <div
              aria-hidden
              className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-brand-primary via-brand-tertiary to-brand-secondary"
            />
            <div className="flex items-start gap-4">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border border-brand-primary/20 bg-brand-primary/10 text-brand-primary">
                <MessageSquareHeart className="h-6 w-6" />
              </span>
              <div>
                <h1 className="text-2xl font-bold text-brand-on-surface md:text-3xl">
                  iLaw - Đóng góp ý kiến trải nghiệm
                </h1>
                <p className="mt-2 text-sm leading-relaxed text-brand-on-surface-variant">
                  Hệ thống iLaw hiện đang trong giai đoạn thử nghiệm Beta. Để nâng cao chất lượng
                  dịch vụ, chúng tôi mong muốn nhận được những cảm nhận thực tế của bạn sau khi trải
                  nghiệm ứng dụng. Vui lòng dành vài phút để hoàn thành bảng khảo sát toàn diện này.
                  Chân thành cảm ơn bạn!
                </p>
                <p className="mt-3 flex items-center gap-1.5 text-xs font-medium text-brand-secondary/80">
                  <span>* Bắt buộc</span>
                </p>
              </div>
            </div>
          ...
          {validationErrors.length > 0 && (
            <div className="mb-6 rounded-xl border border-brand-error/30 bg-brand-error-container/30 p-4 text-sm text-brand-error duration-200 animate-in fade-in-50">
              <p className="mb-2 font-semibold">Vui lòng hoàn thành các trường sau:</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* ========================================================
             ...
             ======================================================== */}
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                <span className="text-xs font-bold uppercase tracking-wider text-brand-tertiary">
                  Phần 1
                </span>
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">
                  Thông tin người dùng & cách tiếp cận pháp luật
                </h2>
              </div>

              {/* Câu 1 */}
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                ...
                <div className="space-y-2.5">
                  {Q1_OPTIONS.map((opt) => (
                    <label
                      key={opt}
                      className="flex cursor-pointer items-start gap-3 rounded-lg border border-brand-outline-variant/10 bg-white/[0.01] p-3 transition-colors hover:bg-white/[0.04]"
                    >
                      <input
                        type="radio"
                        name="q1"
                        ...
                        className="h-4.5 w-4.5 mt-0.5 shrink-0 cursor-pointer border-brand-outline-variant/40 bg-brand-surface text-brand-primary accent-brand-primary focus:ring-brand-primary/40"
                      />
                  ...
                  <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-brand-outline-variant/10 bg-white/[0.01] p-3 transition-colors hover:bg-white/[0.04]">
                    <input
                      ...
                      className="h-4.5 w-4.5 mt-1 shrink-0 cursor-pointer border-brand-outline-variant/40 bg-brand-surface text-brand-primary accent-brand-primary focus:ring-brand-primary/40"
                    />
                    <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center">
                      <span className="shrink-0 text-sm text-brand-on-surface-variant">
                        Mục khác:
                      </span>
                      <input
                        type="text"
                        ...
                        className={cn(
                          ...
                          q1IsOther
                            ...
                            : 'cursor-not-allowed border-brand-outline-variant/20 opacity-50',
                        )}
                      />
                    </div>
                  </label>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  2. Trước khi sử dụng iLaw, mức độ hiểu biết của bạn về pháp luật là:{' '}
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  3. Khi sử dụng iLaw, bạn đã thử hỏi hoặc quan tâm đến lĩnh vực pháp luật nào?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q3_OPTIONS.map((opt) => (
                    ...
                    >
                      <input
                        ...
                        className="h-4.5 w-4.5 mt-0.5 shrink-0 cursor-pointer rounded border-brand-outline-variant/40 bg-brand-surface text-brand-primary accent-brand-primary focus:ring-brand-primary/40"
                      />
                      <span className="text-sm text-brand-on-surface-variant">{opt}</span>
                    </label>
                  ))}
                  ...
                  <label className="flex cursor-pointer items-start gap-3 rounded-lg border border-brand-outline-variant/10 bg-white/[0.01] p-3 transition-colors hover:bg-white/[0.04]">
                    <input
                      ...
                      className="h-4.5 w-4.5 mt-1 shrink-0 cursor-pointer rounded border-brand-outline-variant/40 bg-brand-surface text-brand-primary accent-brand-primary focus:ring-brand-primary/40"
                    />
                    <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center">
                      <span className="shrink-0 text-sm text-brand-on-surface-variant">
                        Mục khác:
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  4. Trước khi dùng iLaw, bạn thường tìm thông tin pháp luật bằng cách nào?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q4_OPTIONS.map((opt) => (
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  5. Bạn đã trải nghiệm những phần nào trên website iLaw?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q5_OPTIONS.map((opt) => (
            ...
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">
                  Cảm nhận về giao diện và khả năng sử dụng
                </h2>
              </div>
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  6. Sau khi trải nghiệm, ấn tượng chung của bạn về website iLaw là:{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
                      Rất không tốt
                    </span>
                    <div className="flex max-w-[320px] flex-1 justify-between">
                      {[1, 2, 3, 4, 5].map((val) => (
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  7. Giao diện website iLaw có dễ nhìn, rõ ràng và tạo cảm giác chuyên nghiệp không?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  8. Bạn có dễ hiểu iLaw đang cung cấp những dịch vụ gì không?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  9. Các thao tác chính trên website như bắt đầu trò chuyện, xem tính năng hoặc tìm
                  thông tin có dễ thực hiện không? <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  10. Trong quá trình sử dụng website, điểm nào khiến bạn khó hiểu, bất tiện hoặc
                  chưa hài lòng nhất?
                </label>
                <textarea
                  rows={4}
                  ...
                  className="w-full rounded-xl border border-brand-outline-variant/30 bg-brand-surface-container-lowest px-4 py-3 text-sm text-brand-on-surface placeholder:text-brand-on-surface-variant/40 focus:border-brand-primary focus:outline-none focus:ring-2 focus:ring-brand-primary/20"
                />
              </div>
            </div>
            ...
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">
                  Cảm nhận về tính hữu ích của iLaw
                </h2>
              </div>
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  11. Sau khi trải nghiệm, bạn đánh giá iLaw hữu ích đến mức nào trong việc hỗ trợ
                  tìm hiểu pháp luật? <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  12. Bạn đã sử dụng hoặc hình dung iLaw hữu ích nhất trong tình huống nào sau khi
                  trải nghiệm? <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q12_OPTIONS.map((opt) => (
                    <label
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  13. Câu trả lời hoặc nội dung trên iLaw có được trình bày bằng ngôn ngữ dễ hiểu
                  không? <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  14. Khi trải nghiệm câu trả lời/nội dung tư vấn trên iLaw, bạn nhận thấy những yếu
                  tố nào? <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q14_OPTIONS.map((opt) => (
                    <label
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  15. Trong các phần bạn đã xem hoặc dùng, phần nào tạo ấn tượng tốt nhất với bạn?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q15_OPTIONS.map((opt) => (
            ...
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">
                  Cảm nhận về độ tin cậy và rủi ro khi dùng AI pháp luật
                </h2>
              </div>
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  16. Sau khi trải nghiệm, bạn tin tưởng câu trả lời/nội dung do iLaw cung cấp ở mức
                  nào? <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  17. Yếu tố nào trên iLaw khiến bạn cảm thấy tin tưởng hơn?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q17_OPTIONS.map((opt) => (
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  18. Sau khi sử dụng, bạn lo ngại iLaw có thể đưa ra câu trả lời sai, thiếu căn cứ
                  hoặc chưa cập nhật ở mức nào? <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex items-center justify-between gap-4 px-2 sm:px-6">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  19. Bạn đánh giá thế nào về mức độ dễ tìm và dễ hiểu của phần điều khoản sử
                  dụng/chính sách/cảnh báo trên iLaw?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q19_OPTIONS.map((opt) => (
            ...
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">
                  Cảm nhận về giá trị và khả năng trả phí
                </h2>
              </div>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  20. Sau khi trải nghiệm, bạn có sẵn sàng tiếp tục sử dụng iLaw không?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q20_OPTIONS.map((opt) => (
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  21. Sau khi trải nghiệm, mức giá hàng tháng bạn thấy phù hợp cho iLaw là:{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="space-y-2.5">
                  {Q21_OPTIONS.map((opt) => (
            ...
            <div className="space-y-8 rounded-2xl border border-brand-outline-variant/20 bg-brand-surface-container/35 p-6 shadow-md md:p-8">
              <div>
                ...
                <h2 className="mt-1 text-lg font-bold text-brand-on-surface">Đánh giá tổng thể</h2>
              </div>

              {/* Câu 23 - 1-10 scale */}
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  23. Nhìn chung, bạn đánh giá trải nghiệm sử dụng iLaw ở mức nào?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 overflow-x-auto rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex min-w-[500px] items-center justify-between gap-4 px-2">
                    <span className="select-none text-xs font-medium text-brand-on-surface-variant/80">
                      Rất không hài lòng
                    </span>
                    <div className="mx-4 flex max-w-[480px] flex-1 justify-between">
              ...
              <div className="space-y-4">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  24. Sau khi trải nghiệm, bạn có sẵn sàng giới thiệu iLaw cho người khác không?{' '}
                  <span className="text-brand-secondary">*</span>
                </label>
                <div className="flex flex-col space-y-2 overflow-x-auto rounded-xl border border-brand-outline-variant/5 bg-white/[0.02] p-4">
                  <div className="flex min-w-[500px] items-center justify-between gap-4 px-2">
                    <div className="flex max-w-[80px] select-none flex-col text-xs text-brand-on-surface-variant/80">
                      <span>Chắc chắn không</span>
                      <span>giới thiệu</span>
                    </div>
                    <div className="mx-4 flex max-w-[520px] flex-1 justify-between">
                    ...
                    <div className="flex max-w-[85px] select-none flex-col text-right text-xs text-brand-on-surface-variant/80">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-semibold leading-snug text-brand-on-surface">
                  25. Nếu chỉ được đề xuất một thay đổi để iLaw tốt hơn, bạn muốn thay đổi điều gì
                  nhất? <span className="text-brand-secondary">*</span>
                </label>
                <textarea
                  rows={4}
                  placeholder="Câu trả lời của bạn..."
